				  ...
				  struct v4l2_frmsizeenum *fsize)
{
	...
	if (fsize->index != 0) {
		mtk_v4l2_err("fsize index is not zero. index = %d\n", fsize->index);
		return -EINVAL;
	}
	...
	for (i = 0; i < MTK_MAX_ENC_CODECS_SUPPORT &&
		...
		if (fsize->pixel_format != mtk_venc_framesizes[i].fourcc) {
			mtk_v4l2_debug(0, "pixel_format = %d, fourcc = %d go next\n", 
					fsize->pixel_format, mtk_venc_framesizes[i].fourcc);
			continue;
		}

		mtk_v4l2_debug(0, "pixel_format = %d, fourcc = %d found\n", 
				fsize->pixel_format, mtk_venc_framesizes[i].fourcc);
	...
	mtk_v4l2_err("Did not found matched pixel_format and fourcc\n");
	return -EINVAL;
}
